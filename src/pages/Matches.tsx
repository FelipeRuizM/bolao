...
export function Matches() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-3 py-4 sm:px-4 sm:py-6 space-y-6">
      <h1 className="text-2xl font-bold px-1">Matches</h1>
      ...
      {matches !== null && matches.length === 0 && !error && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 text-center text-slate-400 text-sm">
          No matches yet. Admin needs to import fixtures from the <span className="font-mono">Admin</span> page.
        </div>
      )}

      {grouped === null && !error && <p className="text-slate-400 text-sm">Loading…</p>}
